...
class ChatServer(object):
	...
	def get_client_name(self, client):
		"""Return the name for the client"""
	...
	def run(self):
		...
		while running:
			try:
				...
				printf("Server interrupted by admin")
				sys.exit(-1)
			...
			for sock in readable:
				if sock == self.server:
					...
					self.clients += 1
					print(f"number of clients = {self.clients}")
					...
					msg = f"\n(Connected: New client from {self.get_client_name(client)} make total number of clients = {self.clients}"
					for output in self.outputs:
						send(output, msg)
					...
					try:
						data = receive(sock)
						if data:
							# print(f"data from '{sock.fileno()}' received: {data}")
